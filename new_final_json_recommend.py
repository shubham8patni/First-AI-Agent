...
#     query_text = travel_data["answer"]# f"Find {product_name} plan and add-on info based on the mentioned scenario : {travel_data["answer"]}"
#     response = retrieve_insurance_info(query=query_text)
...
#     recommendation = chain.run(destination=destination, tavily_summary=response, product_name=product_name)
...
def recommend_mock():
    ...
    # Extract keywords from the event details
    keywords = extract_keywords(event_details)
    print("\n========== Extracted Keywords ==========\n", keywords)
    
    # Build a combined query using destination and extracted keywords
    combined_query = f"Find suitable policy plan and add-on for {keywords}"
    # Retrieve context using the existing retrieval function
    retrieved_context = retrieve_insurance_info(query=combined_query)
    print("\n========== Retrieved Context for Recommendation ==========\n", retrieved_context)
    
    if "I do not have enough information" in retrieved_context:
        return jsonify({"error": "Failed to retrieve relevant policy data."}), 404

    # Recommendation prompt template remains unchanged.
    # template = """You are an AI travel insurance assistant. Your goal is to recommend the best travel insurance product (Zurich or Sompo) based on the latest travel insights.

    # ### Latest Travel Insights:
    # - {event_details}

    # **Recommended Plan for {destination}:**
    # - **Provider:** [Zurich/Sompo]
    # - **Plan Name:** [Plan Name]
    # - **Key Benefits:** 
    #   - [Benefits]
    ...
    # {add_ons}
    # """
   # Define a new, simpler prompt template that uses the retrieved context and the raw event details.
    recommendation_template = """You are an AI travel insurance assistant. Your goal is to recommend the best travel insurance product (Zurich or Sompo) based solely on the following information.
        ...
        Based on the above, provide a clear, detailed recommendation that includes:
        - The recommended provider (e.g., Zurich or Sompo)
        - The recommended plan name
        - Key benefits that address the risks mentioned in the event details
        - Additional add-on recommendations if applicable

        Explain your reasoning clearly in your response.

        Response:
        """
    prompt_mock = PromptTemplate(
        input_variables=["destination", "event_details", "retrieved_context"],
        template=recommendation_template
    )

    # Use LLM chain to generate the recommendation.
    chain = LLMChain(llm=llm2, prompt=prompt_mock)
    ...
    if not recommendation.strip():
        return jsonify({"error": "The recommendation output was empty."}), 500

    return jsonify({"recommendation": recommendation})


def extract_keywords(event_data: str) -> str:
    keyword_prompt = PromptTemplate(
        input_variables=["event_data"],
        template = """
        You are an AI insurance assistant. From the following event description, extract only the keywords that indicate risks or events that could affect travel, such as weather conditions, natural disasters, concerts, holidays, disease outbreaks, and similar events. Ignore any information about destinations, dates, or general vacation activities.

        Event Description: {event_data}

        Extracted Keywords (comma-separated):
        """
    )
    chain = LLMChain(llm=llm2, prompt=keyword_prompt)
    result = chain.invoke({"event_data": event_data})
    keywords = result.get("text", "")
    # Optionally, you could perform post-processing to ensure only risk-related terms remain.
    # For example, you might filter out words that look like dates or location names.
